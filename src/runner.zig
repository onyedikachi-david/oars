...
/// The terminal's 80ms poll loop turns these into a continuous stream
/// (a bridge dispatch plus a re-published frame every tick), drowning
/// out anything useful. They stay available under -Dtrace=all.
const noisy_records = [_][]const u8{ "platform.event", "runtime.frame", "bridge.dispatch" };
