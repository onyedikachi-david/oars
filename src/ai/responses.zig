...
pub fn buildContinuationRequest(
    ...
    if (!validText(model, 256) or !validText(instructions, types.max_message_bytes) or call_id.len == 0 or call_id.len > 128) return error.InvalidInput;
...
pub const State = struct {
    ...
    pub fn consume(self: *State, event: sse.Event) Error!void {
        ...
        if (std.mem.eql(u8, event_type, "response.created")) {
            ...
            if (!std.unicode.utf8ValidateSlice(parsed.value.delta) or self.function_call_arguments.items.len + parsed.value.delta.len > types.max_structured_output_bytes) return error.OutputTooLarge;
    ...
    fn addItem(self: *State, bytes: []const u8) Error!void {
        ...
        if (std.mem.eql(u8, parsed.value.item.type, "function_call")) {
            ...
            if (parsed.value.item.call_id) |cid| {
                self.active_function_call_call_id = try self.allocator.dupe(u8, cid);
            }
    ...
    fn complete(self: *State) Error!void {
        ...
        if (self.function_call_count == 1) {
            ...
            const provider_call_id = if (self.active_function_call_call_id) |cid|
                try self.allocator.dupe(u8, cid)
            else
                try self.allocator.dupe(u8, self.active_function_call_id orelse "");
...
test "Responses native_function mode parses function call across transport splits" {
    ...
    const stream = try std.fmt.allocPrint(std.testing.allocator, "event: response.created\ndata: {{\"type\":\"response.created\",\"response\":{{\"id\":\"resp_2\"}}}}\n\n" ++
        ...
        "event: response.output_item.done\ndata: {{\"type\":\"response.output_item.done\",\"item\":{{\"id\":\"call_1\",\"type\":\"function_call\",\"call_id\":\"call_xyz\",\"name\":\"run_server_command\"}}}}\n\n" ++
        "event: response.completed\ndata: {{\"type\":\"response.completed\",\"response\":{{\"status\":\"completed\"}}}}\n\n", .{ std.json.fmt(args_doc, .{}), std.json.fmt(args_doc, .{}) });
    defer std.testing.allocator.free(stream);
...
test "Responses native_function mode parses preamble text before function call" {
    ...
    const stream = try std.fmt.allocPrint(std.testing.allocator, "event: response.created\ndata: {{\"type\":\"response.created\"}}\n\n" ++
        ...
        "event: response.output_item.done\ndata: {{\"type\":\"response.output_item.done\",\"item\":{{\"id\":\"call_2\",\"type\":\"function_call\",\"call_id\":\"call_abc\",\"name\":\"run_server_command\"}}}}\n\n" ++
...
test "Responses rejects invalid tool names duplicate tool calls and mismatched arguments" {
    ...
    try state2.consume(.{ .name = "response.output_item.added", .data = "{\"type\":\"response.output_item.added\",\"item\":{\"id\":\"call_1\",\"type\":\"function_call\",\"name\":\"run_server_command\"}}" });
    // Duplicate tool call
    try std.testing.expectError(error.MultipleStructuredOutputs, state2.consume(.{
        .name = "response.output_item.added",
        .data = "{\"type\":\"response.output_item.added\",\"item\":{\"id\":\"call_2\",\"type\":\"function_call\",\"name\":\"run_server_command\"}}",
    }));
    ...
    try state3.consume(.{ .name = "response.output_item.added", .data = "{\"type\":\"response.output_item.added\",\"item\":{\"id\":\"call_1\",\"type\":\"function_call\",\"name\":\"run_server_command\"}}" });
