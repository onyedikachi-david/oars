...
pub fn validate(input: JobInput, schedule_credentials: ?[]const u8) SaveError!void {
    // Ids are generated by the store; edits are verified there.
    ...
    if (input.destination.prefix.len > max_prefix_len or hasControlChars(input.destination.prefix)) return error.InvalidDestination;
    if (provider.needsEndpoint() and !validEndpoint(input.destination.endpoint)) return error.InvalidEndpoint;
...
/// cannot overlap itself (spec 10 §9).
pub fn wrapperScript(
    ...
    const header = try std.fmt.allocPrint(allocator,
        \\#!/bin/sh
        \\# oars:job:{s}
        \\# Generated by Oars (spec 10). Do not edit; re-saving the job rewrites this.
        \\STATE_DIR={s}/{s}
        ...
        \\  EXIT=$?
        \\  END=$(date +%s)
        \\  printf '{{\"job_id\":\"{s}\",\"ts\":%s,\"exit\":%s,\"started_at\":%s,\"finished_at\":%s}}\n' "$TS" "$EXIT" "$START" "$END" > "$STATUS"
        \\) 9>"/tmp/oars-backup-{s}.lock"
        \\exit 0
        \\
    , .{ job_id, state_dir, job_id, rclone_invocation, job_id, job_id });
...
pub const JobStore = struct {
    ...
    pub fn save(self: *JobStore, io: std.Io, input: JobInput, schedule_credentials: ?[]const u8, now_ns: i64) SaveError!Job {
        ...
        if (input.id) |i| {
            ...
            id_owned = std.fmt.allocPrint(self.allocator, "bk-{d}", .{self.next_id}) catch return error.OutOfMemory;
            self.next_id +%= 1;
            id = id_owned.?;
        }
        var is_edit = false;
        ...
        if (is_edit) {
            for (loaded.parsed.value) |j| {
                if (std.mem.eql(u8, j.id, id)) created_at = j.created_at_ns;
            }
        }
...
pub const Runs = struct {
    ...
    pub fn start(self: *Runs, job_id: []const u8, server_id: []const u8, source: []const u8, now_ns: i64) !*LiveRun {
        ...
        const id = try std.fmt.bufPrint(&id_buf, "run-{d}", .{self.next_id});
        self.next_id +%= 1;
        run.* = .{
            .record = .{
