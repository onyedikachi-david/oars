...
export function ChatTool({
  ...
  const isAwaitingApproval = state === "awaiting-approval" || state === "approval-requested";
  ...
  return <section className={`chat-tool is-${state}`} aria-label={`${title}: ${stateCopy[state]}`}>
    <header className="chat-tool-header">
      ...
      <span className="chat-tool-state">{state === "running" || state === "preparing" ? <span className="chat-tool-spinner" /> : state === "completed" ? <CheckCircle2 /> : state === "canceled" ? <CircleStop /> : state === "recovery-required" || destructive ? <ShieldAlert /> : <span className="chat-tool-dot" />}{stateCopy[state]}</span>
    </header>
    ...
    <div className="chat-tool-body">
      ...
      {destructive && isAwaitingApproval && <label className="chat-tool-warning"><input type="checkbox" checked={destructiveAck} onChange={(event) => onAckChange?.(event.target.checked)} /><span><strong>This command can change or remove server data.</strong> I reviewed the destructive warning, exact command, and target.</span></label>}

      {isAwaitingApproval && <div className="chat-tool-actions">
      ...
      {state === "completed" && onSummarize && <div className="chat-tool-actions"><Button size="sm" variant="outline" onClick={onSummarize} disabled={busy}>Explain this result</Button>{extraActions}</div>}
    </div>
  </section>;
}
