...
const app_version = "0.5.0"; // x-release-please-version

// ---------------------------------------------------------------------------
// Vendored C libraries (built with zig cc, hermetic, no system deps):
//   - mbedtls : TLS/crypto backend for libssh2 (Apache-2.0)
//   - libssh2 : SSH transport, channels, exec, SFTP, SCP (BSD-3)
// Source lists mirror each project's CMake/Makefile lists.
// ---------------------------------------------------------------------------

const mbedtls_sources = [_][]const u8{
    // src_crypto (library/CMakeLists.txt)
    ...
    "ecdsa.c",                      "ecjpake.c",            "ecp.c",                         "ecp_curves.c",          "ecp_curves_new.c",                       "entropy.c",
    "entropy_poll.c",               "error.c",              "gcm.c",                         "hkdf.c",                "hmac_drbg.c",                            "lmots.c",
    "lms.c",                        "md.c",                 "md5.c",                         "memory_buffer_alloc.c", "nist_kw.c",                              "oid.c",
    "padlock.c",                    "pem.c",                "pk.c",                          "pk_ecc.c",              "pk_wrap.c",                              "pkcs12.c",
    "pkcs5.c",                      "pkparse.c",            "pkwrite.c",                     "platform.c",            "platform_util.c",                        "poly1305.c",
    "psa_crypto.c",                 "psa_crypto_aead.c",    "psa_crypto_cipher.c",           "psa_crypto_client.c",   "psa_crypto_driver_wrappers_no_static.c", "psa_crypto_ecp.c",
    "psa_crypto_ffdh.c",            "psa_crypto_hash.c",    "psa_crypto_mac.c",              "psa_crypto_pake.c",     "psa_crypto_rsa.c",                       "psa_crypto_se.c",
    ...
    // src_x509
               "pkcs7.c",               "x509.c",                                 "x509_create.c",
    "x509_crl.c",                   "x509_crt.c",           "x509_csr.c",                    "x509write.c",           "x509write_crt.c",                        "x509write_csr.c",
    // src_tls
    "debug.c",                      "mps_reader.c",         "mps_trace.c",                   "net_sockets.c",         "ssl_cache.c",                            "ssl_ciphersuites.c",
    "ssl_client.c",                 "ssl_cookie.c",         "ssl_debug_helpers_generated.c", "ssl_msg.c",             "ssl_ticket.c",                           "ssl_tls.c",
    "ssl_tls12_client.c",           "ssl_tls12_server.c",   "ssl_tls13_keys.c",              "ssl_tls13_server.c",    "ssl_tls13_client.c",                     "ssl_tls13_generic.c",
};

const libssh2_sources = [_][]const u8{
    // src/Makefile.inc CSOURCES, plus the mbedTLS crypto backend
    "agent.c",               "bcrypt_pbkdf.c", "channel.c",   "comp.c",      "chacha.c",
    "cipher-chachapoly.c",   "crypt.c",        "crypto.c",    "global.c",    "hostkey.c",
    "keepalive.c",           "kex.c",          "knownhost.c", "mac.c",       "mbedtls.c",
    "misc.c",                "packet.c",       "pem.c",       "poly1305.c",  "publickey.c",
    "scp.c",                 "session.c",      "sftp.c",      "transport.c", "userauth.c",
    "userauth_kbd_packet.c", "version.c",
};

/// Builds the vendored mbedTLS and libssh2 static libraries.
/// Crypto always compiles ReleaseFast regardless of the app's mode: a
/// Debug app must not ship a Debug crypto core, and the small build-time
/// cost is paid once per cache.
fn buildVendoredLibraries(b: *std.Build, target: std.Build.ResolvedTarget) struct {
    mbedtls: *std.Build.Step.Compile,
    ssh2: *std.Build.Step.Compile,
} {
    const c_flags = [_][]const u8{ "-w", "-fno-sanitize=undefined" };

    const mbedtls_mod = b.createModule(.{ .target = target, .optimize = .ReleaseFast });
    mbedtls_mod.linkSystemLibrary("c", .{});
    mbedtls_mod.addIncludePath(b.path("third_party/mbedtls/include"));
    mbedtls_mod.addCSourceFiles(.{
        .root = b.path("third_party/mbedtls/library"),
        .files = &mbedtls_sources,
        .flags = &c_flags,
    });
    const mbedtls = b.addLibrary(.{ .linkage = .static, .name = "mbedtls", .root_module = mbedtls_mod });

    const ssh2_mod = b.createModule(.{ .target = target, .optimize = .ReleaseFast });
    ssh2_mod.linkSystemLibrary("c", .{});
    // src/c first: our libssh2_config.h must win over any generated copy.
    ssh2_mod.addIncludePath(b.path("src/c"));
    ssh2_mod.addIncludePath(b.path("third_party/libssh2/include"));
    ssh2_mod.addIncludePath(b.path("third_party/libssh2/src"));
    ssh2_mod.addIncludePath(b.path("third_party/mbedtls/include"));
    ssh2_mod.addCMacro("LIBSSH2_MBEDTLS", "1");
    // Autotools-style build: our src/c/libssh2_config.h is the config.h.
    ssh2_mod.addCMacro("HAVE_CONFIG_H", "1");
    ssh2_mod.addCSourceFiles(.{
        .root = b.path("third_party/libssh2/src"),
        .files = &libssh2_sources,
        .flags = &c_flags,
    });
    const ssh2 = b.addLibrary(.{ .linkage = .static, .name = "ssh2", .root_module = ssh2_mod });
    ssh2_mod.linkLibrary(mbedtls);

    return .{ .mbedtls = mbedtls, .ssh2 = ssh2 };
}

/// Links the vendored SSH stack into an app module and exposes the C
/// headers to @cImport sites in that module.
fn linkSshStack(b: *std.Build, app_mod: *std.Build.Module, vendored: anytype) void {
    app_mod.linkLibrary(vendored.ssh2);
    app_mod.linkLibrary(vendored.mbedtls);
    app_mod.addIncludePath(b.path("third_party/libssh2/include"));
    app_mod.addIncludePath(b.path("third_party/mbedtls/include"));
}
